...
const PERMISSION_DEFS: Array<{ key: string; group: string; description: string }> = [
  ...
  // team (public-facing team members managed from dashboard)
  ...
  // mission vision values page + core values
...
const MENU_SEED: Array<{
  ...
  // Blog Management parent + children
...
async function seed() {
  ...
  for (const p of PERMISSION_DEFS) {
    const id = randomUUID();
    permissionIdByKey[p.key] = id;
    db.insert(schema.permissions)
      .values({ id, key: p.key, group: p.group, description: p.description })
      .onConflictDoNothing()
      .run();
  }

  // re-read so we have actual ids (handles re-runs)
  const dbRoles = db.select().from(schema.roles).all();
  const dbPerms = db.select().from(schema.permissions).all();
  ...
  // First pass: insert any missing menus (no parent set yet).
  ...
  // Second pass: re-sync parentId, sortOrder, href, and icon on all seeded rows
  // so re-running the seed brings the hierarchy into the desired state.
  const dbMenus = db.select().from(schema.menus).all();
  ...
  const demoUsers = [
    {
      email: "admin@thebrandghar.com",
      password: "admin123",
      fullName: "Sagar (Super Admin)",
      role: "super_admin",
    },
    {
      email: "content@thebrandghar.com",
      password: "content123",
      fullName: "Content Manager",
      role: "content_manager",
    },
    {
      email: "sales@thebrandghar.com",
      password: "sales123",
      fullName: "Sales Manager",
      role: "sales_manager",
    },
    {
      email: "viewer@thebrandghar.com",
      password: "viewer123",
  ...
  // Only seed if the table is empty — don't clobber later edits.
  ...
  for (const c of BLOG_CATEGORIES_SEED) {
    db.insert(schema.blogCategories)
      .values({
        id: randomUUID(),
        name: c.name,
        slug: c.slug,
  ...
  const BLOG_SEED = [
    {
      ...
      content:
        "Most brands treat social media as a popularity contest. Followers, likes, and impressions feel good — but they don't pay the bills.\n\nThe brands winning on social in 2026 treat every post as a step in a funnel. Awareness at the top, engagement in the middle, conversion at the bottom. And they measure what matters: qualified leads, booked calls, and closed deals.\n\nThis post walks through the exact framework we use at Brandghar to turn a sleepy Instagram feed into a predictable pipeline.",
    ...
    },
    {
      ...
      content:
        "If your website isn't ranking, it's almost never because Google doesn't like you. It's because you're making one of these five mistakes.\n\n1. Targeting keywords with no intent. 2. Thin content that adds no value. 3. Ignoring technical health. 4. Building no internal links. 5. Forgetting about mobile entirely.\n\nFix these five and you'll see movement within 60 days.",
      category: "SEO",
      readTimeMinutes: 6,
    },
    {
      ...
      content:
        "A logo is a signature. A visual identity system is a vocabulary. The first is recognizable. The second is ownable.",
    ...
    },
    {
      ...
      content:
        "Meta captures demand before it exists. Google captures demand that already exists. Neither is universally better — they're built for different stages of the buying journey.",
      category: "Digital Marketing",
      readTimeMinutes: 7,
    },
    {
      ...
      content:
        "Batching is the secret weapon of every content team that actually ships. Here's our 2-hour monthly workflow.",
    ...
    },
    {
      ...
      content:
        "The playbook that works for a Silicon Valley SaaS company doesn't always work for a Kathmandu boutique. Here's what does.",
      category: "Business Growth",
      readTimeMinutes: 6,
    },
  ];

  const existingBlog = db.select().from(schema.blogPosts).all();
  if (existingBlog.length === 0) {
    let order = 0;
    for (const p of BLOG_SEED) {
      db.insert(schema.blogPosts)
        .values({
          id: randomUUID(),
          slug: p.slug,
          title: p.title,
  ...
  const existingClients = db.select().from(schema.ourCompanyClients).all();
  if (existingClients.length === 0) {
    for (const c of CLIENT_SEED) {
      db.insert(schema.ourCompanyClients)
        .values({
          id: randomUUID(),
          name: c.name,
          logoUrl: c.logoUrl,
  ...
  const existingValues = db.select().from(schema.coreValues).all();
  if (existingValues.length === 0) {
    for (const v of CORE_VALUES_SEED) {
      db.insert(schema.coreValues)
        .values({
          id: randomUUID(),
          title: v.title,
  ...
  if (!existingContact) {
    db.insert(schema.contactPage)
      .values({
        ...
        instagramUrl:
          "https://www.instagram.com/thebrandghar?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==",
        facebookUrl: "https://www.facebook.com/thebrandghar",
        tiktokUrl: "https://www.tiktok.com/@the.brand.ghar",
        linkedinUrl:
          "https://www.linkedin.com/company/brandghar/?viewAsMember=true",
  ...
  const existingTrust = db.select().from(schema.contactTrustPoints).all();
  if (existingTrust.length === 0) {
    for (const tp of TRUST_POINTS_SEED) {
      db.insert(schema.contactTrustPoints)
        .values({
          id: randomUUID(),
          text: tp.text,
          icon: tp.icon,
